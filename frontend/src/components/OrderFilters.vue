...
const props = defineProps({ modelValue: String, search: String, counts: Object, visibleOrders: Array, selected: Array });
...
<template>
  <div class="filter-bar">
    <label class="select-all">
      ...
      <span>Выбрать всё на странице</span>
      <small v-if="selectedVisibleCount">{{ selectedVisibleCount }} / {{ visibleIds.length }}</small>
    </label>
    <div class="tabs" role="tablist">
      <button v-for="item in [['all','Все'],['passed','Прошли'],['warning','Предупреждения'],['error','Ошибки'],['waiting_confirmation','Нужно решение']]" :key="item[0]" :class="{ active: modelValue === item[0] }" @click="$emit('update:modelValue', item[0])">{{ item[1] }} <span>{{ counts?.[item[0]] ?? 0 }}</span></button>
    </div>

...
const filters = reactive({ action: "all", date_from: "", date_to: "", search: "" });
  ...
  <main class="history-page">
    ...
    <section v-if="items.length" class="history-list">
      <article v-for="item in items" :key="item.id" class="history-item surface">
        ...
        <div class="history-data"><strong>№ {{ item.order_id }}</strong><span>Клиент: {{ item.customer_id || "—" }}</span><span :class="`history-action ${item.action}`">{{ label[item.action] }}</span><small>{{ formatDate(item.created_at) }}</small><p v-if="item.comment">{{ item.comment }}</p></div>
      </article>
    </section>

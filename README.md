...
До восьми независимых заказов и до двух превью внутри каждого заказа создаются
параллельно; обычные превью исходников сохраняются до проверки PitStop.
Лимиты можно изменить: `IMAGE_MAGIC_ORDER_WORKERS="8"` — число одновременно
обрабатываемых заказов, `IMAGE_MAGIC_PREVIEW_WORKERS="2"` — превью на заказ.
Значение `1` отключает соответствующую параллельность.

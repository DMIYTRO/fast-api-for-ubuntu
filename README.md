...
- минимум 270 DPI после любой коррекции;
...
- увеличение разрешается только при итоговом эффективном разрешении не ниже 270 DPI;
...
По умолчанию заказы обрабатываются по одному в порядке обнаружения:
так сохраняется порядок подготовки превью и не перегружаются внешние инструменты.
`IMAGE_MAGIC_ORDER_WORKERS` задаёт число одновременно обрабатываемых заказов;
значение больше `1` может изменить порядок завершения и загрузку инструментов.
Лимит не может превышать число доступных экземпляру ядер.
`IMAGE_MAGIC_PREVIEW_WORKERS` ограничивает создание превью внутри заказа;
обычные превью исходников сохраняются до проверки PitStop.
